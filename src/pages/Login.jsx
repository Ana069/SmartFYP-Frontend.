...
import { findDemoAccount, buildDemoLoginResponse, DEMO_MODE_FLAG } from '../utils/demoMode';
import { 
  LayoutDashboard, 
  Mail, 
  Lock, 
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    // --- DEMO MODE: check local demo accounts first, no backend needed ---
    const demoAccount = findDemoAccount(email, password);
    if (demoAccount) {
      if (demoAccount.role !== role) {
        setError(`Role mismatch: This demo account is ${demoAccount.role}, but you selected ${role}.`);
        setLoading(false);
        return;
      }
      localStorage.setItem(DEMO_MODE_FLAG, 'true');
      localStorage.setItem('activeDashboardRole', role);
      login(buildDemoLoginResponse(demoAccount));
